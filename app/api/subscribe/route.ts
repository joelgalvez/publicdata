...
export async function GET(request) {
    ...
    let endSpan = moment().add(2, 'years').toDate();

    // let searchParams = request.nextUrl.searchParams;    // const all = await getEvents(searchParams, start, end);

    const params = Object.fromEntries(request.nextUrl.searchParams.entries());

    const allEvents = await getEvents(params, startSpan, endSpan);
    const allDays = getDays(allEvents);
    ...
    allDays.map(day => {
        ...
        let startOfDay = moment().startOf('day').add(dayCount, 'days').toDate();
        let endOfDay = moment(startOfDay).add(24, 'hours').toDate();
        ...
        day.events.forEach(event => {
            ...
            let summaryNoSpace = event.summary.replace(/ +(?= )/g, '');
            summaryNoSpace = summaryNoSpace.replace(/(\r\n|\n|\r)/gm, "");

            dayText += `${timeText} - ${event.venue.title.trim()}, ${summaryNoSpace.trim()} \n\n`;
        ...
        if (eventPerDayCount > 0) {
            ...
            let tempDay = startOfDay.getDate();
            ...
            let dayEvent = {
                start: [startOfDay.getFullYear(), startOfDay.getMonth() + 1, tempDay],
                end: [startOfDay.getFullYear(), startOfDay.getMonth() + 1, tempDay2],
